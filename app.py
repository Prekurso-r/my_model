...
st.markdown('<h1 class="main-header">Diabetes Risk Prediction</h1>',
            unsafe_allow_html=True)
st.markdown("""
<div class="info-box">
This application estimates <strong>diabetes risk</strong> using an 
<strong>XGBoost machine learning model</strong> with <strong>explainable AI (SHAP)</strong>.
</div>
""", unsafe_allow_html=True)
...
st.markdown('<h2 class="section-header">Patient Information</h2>',
            unsafe_allow_html=True)
...
st.markdown('<h2 class="section-header">Clinical Measurements</h2>',
            unsafe_allow_html=True)
...
st.markdown('<h2 class="section-header">Prediction</h2>',
            unsafe_allow_html=True)
...
if st.button("Predict Diabetes Risk", use_container_width=True, type="primary"):
    ...
    st.divider()
    st.markdown('<h2 class="section-header">Model Assessment</h2>',
                unsafe_allow_html=True)
    ...
    if prediction == 1:
        st.error("Model Prediction: Diabetes Likely")
    else:
        st.success("Model Prediction: Diabetes Unlikely")

    st.write(interpretation)

    st.markdown('<h2 class="section-header">Why did the model make this prediction?</h2>',
                unsafe_allow_html=True)
...
st.caption(
    "This tool is intended for educational and decision-support use only and does not replace medical advice."
)
